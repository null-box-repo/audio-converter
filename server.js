'use strict';
// .. HTTP server: convert audio via FFmpeg
...
function handleConvert(req, res) {
  ...
  collectBody(req, (err, buf) => {
    ...
    fs.writeFile(inputPath, buf, (werr) => {
      ...
      const codec = AUDIO_CODEC_MAP[fmt.ext] || '-c:a copy';
      const args = ['-i', inputPath, '-vn', '-y', outputPath, ...codec.split(' ')];
...
server.listen(PORT, () => {
  console.log('Audio Converter running on http://localhost:' + PORT);
});
